...
while ($row = $db->sql_fetchrow($result))
{
	...
	if (!isset($user_cache[$poster_id]))
	{
		...
		else
		{
			...
			/*НАЧАЛО: Илья Домышев. 05.02.2015. Получаем имя руководителя.*/
			$db->sql_query("SELECT * FROM ".USERS_TABLE." WHERE `user_id`=$poster_id");
			$poster=$db->sql_fetchrow();
			$poster_boss_id=$poster['mychief_id'];
			$db->sql_query("SELECT * FROM ".USERS_TABLE." WHERE `user_id`=$poster_boss_id");
			$poster_boss_name=$db->sql_fetchfield('username');
			$poster_office_id=$poster['point_id'];
			if($poster_office_id>1){
				$db->sql_query("SELECT * FROM `phpbb_points` WHERE `id`=$poster_office_id");
				$poster_office_name=$db->sql_fetchfield('name');
			}
			$db->sql_freeresult();
			/*КОНЕЦ: Илья Домышев. 05.02.2015. Получаем имя руководителя.*/
			...
			$user_cache[$poster_id] = array(
				'joined'		=> date("d.m.Y", $row['user_regdate']),
				'boss_name'		=> $poster_boss_name,
				'boss_id'		=> $poster_boss_id,
				'office_name'	=> $poster_office_name,
				'office_id'		=> $poster_office_id,
				'occupation'	=> $row['user_occ'],
				'poster_email'	=> $row['user_email'],
				'skype'			=> $row['user_skype'],
				'workmobilephone' => $row['user_workmobilephone'],
				'privatemobilephone' => $row['user_privatemobilephone'],
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'POSTER_BOSS'		=> $user_cache[$poster_id]['boss_name'],
		'U_POSTER_BOSS'		=> "/manager.php?action=show_contact&contact=".$user_cache[$poster_id]['boss_id'],
		'POSTER_OFFICE'		=> $user_cache[$poster_id]['office_name'],
		'U_POSTER_OFFICE'	=> "/manager.php?action=show_point&point=".$user_cache[$poster_id]['office_id'],
		'POSTER_OCCUPATION'	=> $user_cache[$poster_id]['occupation'],
		'POSTER_EMAIL'		=> $user_cache[$poster_id]['poster_email'],
		'POSTER_SKYPE'		=> $user_cache[$poster_id]['skype'],
		'POSTER_WORKMOBILEPHONE'		=> $user_cache[$poster_id]['workmobilephone'],
		'POSTER_PRIVATEMOBILEPHONE'		=> $user_cache[$poster_id]['privatemobilephone'],
