...
<div class='b_i_d'>
{if $show_hr_information==true}<input type='hidden' id='show_hr_information' value='1'>{else}<input type='hidden' id='show_hr_information' value='0'>{/if}
...
{if $user_hire != '0000-00-00'}
	...
	<input id='vacation_transfer_days_number' type='text' value='{$hire_info.transfer_days_number}' class='e_s_t' maxlength='2' onblur='set_transfer_days_number(this.value)' onfocus='say_result("", "")' />&nbsp;д
	...
	{/if}
	<br/>Остаток: <b><span id='vacation_rest'></span></b><br/><br/>
	
	<h4 class='a_i_b'>Больничный</h4>
	Использовано: <span id='sickleave_used_days_number'>{$attendance_info[3]['used_days']}</span>д <span id='sickleave_used_hours_number' {if $attendance_info[3]['used_hours']==0}style='display:none'{/if}>{$attendance_info[3]['used_hours']}</span>{if $attendance_info[3]['used_hours']>0}ч{/if}<br/>
	{if $attendance_info[3]['hours']>0}Когда: {$attendance_info[3]['when']}<br/>{/if}
	Начислено итого: <span id='sickleave_credit_days_number'>{$credits_info.sickleave.credit_days}</span>д <span id='sickleave_credit_hours_number'>{$credits_info.sickleave.credit_hours}</span>ч<br/>
	Остаток: <b><span id='sickleave_rest'></span></b><br/><br/>
