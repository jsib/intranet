...
function user_notification($mode, $subject, $topic_title, $forum_name, $forum_id, $topic_id, $post_id)
{
	...
	//Илья Домышев 14.01.201
	//НАЧАЛО: Ограничить количество получателей уведомления о новом посте
		//Раскомментировать строку ниже, чтобы письмо отправлялось только одному пользователю с соответствующим id
		//$limit_number_recipients="AND u.`user_id`=5871";
	//КОНЕЦ: Ограничить количество получателей уведомления о новом посте
	
	$limit_number_recipients="";
	
	// -- get forum_userids	|| topic_userids
	$sql = 'SELECT u.user_id, u.username, u.user_email, u.user_lang, u.user_notify_type, u.user_jabber
		FROM ' . (($topic_notification) ? TOPICS_WATCH_TABLE : FORUMS_WATCH_TABLE) . ' w, ' . USERS_TABLE . " u
			WHERE w.user_id NOT IN ($sql_ignore_users) $limit_number_recipients
...
function submit_post($mode, $subject, $username, $topic_type, &$poll, &$data, $update_message = true, $update_search_index = true)
{
	...
	{
		//Илья Домышев. Предположительно, эту строчку нужно закомментировать, чтобы не происходило оповещение о посте по email
