...
function template_exists($template_name){
    return file_exists($_SERVER['DOCUMENT_ROOT']."/templates/$template_name.html");
}

//Выполняет замены в строке
function text_replace($match, $replacement, $text){
	//Делаем замену
	$text=str_replace("{".$match."}",$replacement, $text);
	
	//Возвращаем текст с выполненными заменами
	return $text;
}
?>